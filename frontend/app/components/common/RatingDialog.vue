...
async function submit() {
  ...
  try {
    await apiFetch(props.endpoint, { method: 'POST', body: { rating: rating.value, comment: comment.value.trim() || undefined } })
    ...
    // Le backend renvoie déjà des messages français précis (éligibilité, doublon).
...
<template>
  <v-dialog v-model="open" max-width="360">
    <v-card class="pa-4">
      <h2 class="text-h6 mb-3">{{ title }}</h2>
